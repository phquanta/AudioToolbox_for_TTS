...
class Scrolling_Area(Frame, object):
    ...
    def __init__(self, master, width=None, anchor=N, height=None, mousewheel_speed = 2, scroll_horizontally=True, xscrollbar=None, scroll_vertically=True, yscrollbar=None, outer_background=None, inner_frame=Frame, **kw):
        ...
        self.innerframe.pack(anchor=N+W,side=LEFT,expand=True,fill=BOTH)
...
class Data_Cell(Cell):
    def __init__(self, master, variable, anchor=W, bordercolor=None, borderwidth=1, padx=0, pady=0, background=None, foreground=None, font=None):
        ...
        #self._message_widget = Message(self, textvariable=variable, font=font, background=background, foreground=foreground,width=900,justify=LEFT,anchor=W)
        self._message_widget = Label(self, textvariable=variable, font=font, background=background, foreground=foreground,justify=LEFT,anchor=N+W)
        #self._message_widget = Message(self, textvariable=variable, font=font, background=background, foreground=foreground,justify=LEFT,anchor=W)
        self._message_widget.pack(expand=True, padx=padx, pady=pady, anchor=anchor,fill=BOTH)
...
class Header_Cell(Cell):
    def __init__(self, master, text, bordercolor=None, borderwidth=1, padx=0, pady=0, background=None, foreground=None, font=None, anchor=CENTER, separator=True):
        ...
        self._header_label = Label(self, text=text, background=background, foreground=foreground, font=font,width=30,justify=CENTER,anchor=N)
...
class Table(Frame):
    def __init__(self, master, columns, column_weights=None, column_minwidths=None, height=500, minwidth=20, minheight=20, padx=5, pady=5, cell_font=None, cell_foreground="black", cell_background="white", cell_anchor=W, header_font=None, header_background="white", header_foreground="black", header_anchor=CENTER, bordercolor = "#999999", innerborder=True, outerborder=True, stripped_rows=("#EEEEEE", "white"), on_change_data=None, mousewheel_speed = 2, scroll_horizontally=False, scroll_vertically=True):
        ...
        self._head.columnconfigure(0,weight=0)
        self._head.columnconfigure(1,weight=1)
        self._head.columnconfigure(2,weight=3)
        ...
        for j in range(len(columns)):
            ...
            header_cell = Header_Cell(self._head, text=column_name, borderwidth=self._innerborder_width, font=header_font, background=header_background, foreground=header_foreground, padx=self._padx, pady=self._pady, bordercolor=bordercolor, anchor=header_anchor, separator=header_separator)
        ...
        if add_scrollbars:
            ...
            scrolling_area.grid(row=1, column=0, sticky=N+E+W)
            ...
            self._body.pack(fill=BOTH,anchor=N+W,side=LEFT,expand=True)
        ...
        if column_weights is None:
            ...
            for j, weight in enumerate(column_weights):
                print("weight:",weight)
                #self._body.columnconfigure(j, weight=weight)
        ...
        if column_minwidths is not None:
            for j, minwidth in enumerate(column_minwidths):
                ...
                self._head.grid_columnconfigure(j, minsize=minwidth)
        else:
            for j in range(len(columns)):
                header_cell = self._head.grid_slaves(row=0, column=j)[0]
                minwidth = header_cell.winfo_reqwidth()
    ...
    def _append_n_rows(self, n):
        ...
        for i in range(number_of_rows, number_of_rows+n):
            ...
            for j in range(number_of_columns):
                ...
                cell.columnconfigure(j, weight=j)
